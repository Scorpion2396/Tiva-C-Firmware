...
void I2C_Init(I2C_Channel I2C_Ch_Index, uint32_t I2C_Freq_KHz)
{
    ...
    I2C_config[I2C_Index].GPIO_Perif_Addr->DEN        |=   ((1<<(I2C_config[I2C_Index].SCL_Pin)) 
                                                            | (1<<(I2C_config[I2C_Index].SDA_Pin)));                               // DIGITAL ENABLE  FOR PORT A PIN 6 AND PIN 7;        
	
    I2C_config[I2C_Index].GPIO_Perif_Addr->AFSEL      |=   ((1<<(I2C_config[I2C_Index].SCL_Pin)) 
                                                             | (1<<(I2C_config[I2C_Index].SDA_Pin)));                               // ENABLE ALTERNATE FUNCTION SELECT FOR PORT A PIN 6 AND PIN 7               
...
{
    I2C_Begin(slave_address);
    I2C_Transmit_start_byte(slave_memory_address);
    I2C_Transmit_end_byte(data);
}
...
void I2C_Write_Multiple(uint8_t slave_address , uint8_t *data , uint32_t num_of_data)
{
    ...
    I2C_Transmit_start_byte(data[0]);
    
    for(cnt = 1 ; cnt < num_of_data ; cnt++)
    {
        I2C_Transmit_byte(data[cnt]);
    }
