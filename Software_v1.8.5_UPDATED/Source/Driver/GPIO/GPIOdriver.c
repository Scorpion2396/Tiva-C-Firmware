...
GPIOA_Type* const GPIO[6] = { GPIOA , GPIOB , GPIOC , GPIOD , GPIOE , GPIOF };
...
void GPIO_Init(void)
{ 
    // enable clock for GPIO's peripheral
...
void DigitalWrite(GPIO_PORT port_x , GPIO_HI_LO value)
{
    ...
    if(IO_flag == 0) 
    {
        GPIO[port_num]->CR     |=  (1<<(pin_num));
        //GPIO[port_num]->AFSEL  &= ~(1<<(pin_num));
        GPIO[port_num]->DIR    |=  (1<<(pin_num));
        GPIO[port_num]->DEN    |=  (1<<(pin_num));
...
uint8_t DigitalRead(GPIO_PORT port_x , GPIO_PU_PD state)
{
    ...
    if( state == PU )
    {
    	GPIO[port_num]->PUR   |=  (1<<(pin_num));				
    }
    else
    {
    	GPIO[port_num]->PDR   |=  (1<<(pin_num));				
    }
			 	
    Input_Data  = GPIO[port_num]->DATA;      
