...
static void SSI_2_Init(void)
{
	...
	SSI2->CPSR         =   0xA0;				 //(CPSDVSR = 16 = 0x10)  SSInClk = SysClk / CPSDVSR       ==> SSInClk = 16MHz/16 ==>    1MHz
...
{
	volatile unsigned char spi_recieve_data = 0;
...
//  DigitalWrite(PA4, LOW);          /* Make PA4 Chip Selection line (SS) low */
    ...
    SSI2->DR = data;                 /* transmit byte over SSI2Tx line */

    while(SSI2->SR & 0x10);          /* wait until transmit complete */

//  DigitalWrite(PA4, HIGH);         /* keep Chip selection line (PA4) high in idle condition */
    
    spi_recieve_data = SSI2->DR;     /*recieved data byte over SSI2Tx line */
...
{
	DigitalWrite(port_x, LOW);          /* Make PA4 Chip Selection line (SS) low */
}

void Disable_Chip_Select(GPIO_PORT port_x)
{
	DigitalWrite(port_x, HIGH);         /* keep Chip selection line (PA4) high in idle condition */
}
